...
/**
 * Deals with EQUIPMOD token
 */
public class EquipmodToken extends AbstractTokenWithSeparator<Equipment> implements
		CDOMPrimaryToken<Equipment>
{

	private static final Class<EquipmentModifier> EQUIPMOD_CLASS = EquipmentModifier.class;

	private static final String EQUIPMOD_WEIGHT = "_WEIGHTADD";

	private static final String EQUIPMOD_DAMAGE = "_DAMAGE";
	...
	{
		StringTokenizer pipeTok = new StringTokenizer(value, Constants.DOT);
		EquipmentHead head = eq.getEquipmentHead(1);
		while (pipeTok.hasMoreTokens())
		{
			String modInfo = pipeTok.nextToken();
...
//		boolean allowPre = true;
			...
			{
				Logging.deprecationPrint("'NONE' EquipMod in " + getTokenName()
			...
			StringTokenizer aTok = new StringTokenizer(modInfo, Constants.DOT);

			// The type of EquipMod, eg: ABILITYPLUS
			String equipModKey = aTok.nextToken();
			if (equipModKey.equals(EQUIPMOD_WEIGHT))
			{
			...
			if (equipModKey.equals(EQUIPMOD_DAMAGE))
			{
				if (aTok.hasMoreTokens())
				{
			...
			CDOMSingleRef<EquipmentModifier> ref = context.getReferenceContext()
					.getCDOMReference(EQUIPMOD_CLASS, equipModKey);
			...
			context.getObjectContext().addToList(head, ListKey.EQUIPMOD_INFO, modref);
		}
	...
	public String[] unparse(LoadContext context, Equipment eq)
	{
		...
		if (damage != null)
		{
			set.add(EQUIPMOD_DAMAGE + Constants.PIPE + damage);
		}
		BigDecimal weight = obj.getObject(eq, ObjectKey.WEIGHT_MOD);
		if (weight != null)
		{
			set.add(EQUIPMOD_WEIGHT + Constants.PIPE
